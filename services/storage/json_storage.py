...
class JSONStorageBackend(StorageBackend):
    ...
    def __init__(self, file_path: Path, auth_keys_path: Path | None = None, public_works_path: Path | None = None):
        self.file_path = file_path
        self.auth_keys_path = auth_keys_path or file_path.with_name("auth_keys.json")
        self.public_works_path = public_works_path or file_path.with_name("public_works.json")
        self.file_path.parent.mkdir(parents=True, exist_ok=True)
        self.auth_keys_path.parent.mkdir(parents=True, exist_ok=True)
        self.public_works_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    def load_public_works(self) -> list[dict[str, Any]]:
        """从 JSON 文件加载公开作品数据"""
        return self._load_json_list(self.public_works_path)

    def save_public_works(self, items: list[dict[str, Any]]) -> None:
        """保存公开作品数据到 JSON 文件"""
        self._save_json_list(self.public_works_path, items)
    ...
    def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "public_works_file_exists": self.public_works_path.exists(),
                "public_works_file_path": str(self.public_works_path),
            }
